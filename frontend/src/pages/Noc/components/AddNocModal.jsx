  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        expenseBreakup: formData.expenseBreakup.filter((item) => item.name && item.amount && parseFloat(item.amount) > 0)
      }
          ...
          <div className='flex-1 overflow-y-auto p-3 md:p-6'>
            ...
            <div className='mt-4 pt-4 border-t border-purple-200'>
              <button
                ...
                className='w-full flex items-center justify-between p-3 bg-purple-50 hover:bg-purple-100 rounded-lg transition group'
              >
                <span className='text-sm md:text-base font-bold text-gray-800 group-hover:text-purple-700 transition-colors'>
                  Additional Details
                </span>
                <svg className={`w-5 h-5 text-purple-500 transition-transform duration-200 ${showAdditionalDetails ? 'rotate-180' : ''}`} fill='none' stroke='currentColor' viewBox='0 0 24 24'>
