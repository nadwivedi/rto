...
const ProfessionalTaxDetailModal = ({ isOpen, onClose, record }) => {
  ...
  const docUrl = record.professionalTaxDocument
    ? record.professionalTaxDocument.startsWith('http')
      ? record.professionalTaxDocument
      : `${API_URL}${record.professionalTaxDocument}`
    : null
      ...
      <div className='bg-white rounded-xl md:rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] overflow-hidden flex flex-col'>
        <div className='bg-gradient-to-r from-indigo-600 to-violet-600 p-3 md:p-4 text-white flex-shrink-0'>
          <div className='flex justify-between items-center'>
            <div>
              <h2 className='text-lg md:text-2xl font-bold'>Professional Tax Details</h2>
              <p className='text-indigo-100 text-xs md:text-sm mt-1'>{record.ownerName || 'N/A'}</p>
            </div>
        ...
        <div className='flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6'>
          <div className='bg-gradient-to-r from-indigo-50 to-blue-50 border-2 border-indigo-200 rounded-xl p-4 md:p-5'>
            ...
            <div className='grid grid-cols-2 gap-3 md:gap-4 text-xs md:text-sm'>
              ...
              <div>
                <span className='block text-gray-500 font-medium'>GRN No</span>
                <span className='block font-bold text-gray-800 mt-0.5'>{record.grnNo || 'N/A'}</span>
              </div>
              <div>
                <span className='block text-gray-500 font-medium'>Dealer TIN / PAN</span>
                <span className='block font-bold text-gray-800 mt-0.5'>{record.dealerTIN || 'N/A'}</span>
              </div>
            </div>
          </div>
              ...
              <div className='w-full max-h-72 rounded-lg overflow-hidden border border-sky-200 bg-white'>
                {isPDF ? (
                  <div className='flex flex-col items-center justify-center p-8'>
                    ...
                    <p className='text-sm font-semibold text-gray-600 mb-2'>Professional Tax Document (PDF)</p>
                    <a href={docUrl} target='_blank' rel='noopener noreferrer' className='px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition font-semibold text-sm'>View PDF</a>
                  </div>
                ) : isImage ? (
                  <img src={docUrl} alt='Professional Tax Document' className='w-full h-auto max-h-72 object-contain mx-auto' onError={() => setDocError(true)} />
                ) : null}
        ...
        <div className='border-t border-gray-200 p-3 md:p-4 bg-gray-50 flex justify-end flex-shrink-0'>
          <button onClick={onClose} className='px-6 py-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-lg hover:shadow-lg font-semibold transition cursor-pointer'>Close</button>
        </div>
      </div>
    </div>
  )
...
export default ProfessionalTaxDetailModal
