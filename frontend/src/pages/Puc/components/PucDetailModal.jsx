...
const PucDetailModal = ({ isOpen, onClose, puc }) => {
  ...
  const documentUrl = useMemo(() => {
    if (!puc?.pucDocument) return ''
    return puc.pucDocument.startsWith('data:')
      ? puc.pucDocument
  ...
  const isPdfDocument = useMemo(() => {
    ...
    return documentUrl.toLowerCase().includes('.pdf') || (puc?.pucDocumentType || '').includes('pdf')
  }, [documentUrl, puc?.pucDocumentType])

  if (!isOpen || !puc) return null
  ...
  const handleShareDocument = async () => {
    ...
    try {
      ...
      const fallbackName = puc.pucDocumentName || `${puc.vehicleNumber || 'puc-document'}${isPdfDocument ? '.pdf' : '.jpg'}`
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: `PUC Document - ${puc.vehicleNumber}`,
          text: `PUC document for ${puc.vehicleNumber}`,
          files: [file]
        })
        return
      }
      ...
      const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`PUC document for ${puc.vehicleNumber}: ${documentUrl}`)}`
      window.open(whatsappUrl, '_blank', 'noopener,noreferrer')
    } catch (error) {
      console.error('Error sharing PUC document:', error)
        ...
        <div className='bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 p-3 md:p-4 text-white shadow-lg'>
          <div className='flex justify-between items-center gap-2'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                <h2 className='text-base md:text-xl font-bold'>PUC Certificate Details</h2>
                <p className='text-xs md:text-sm text-white/90 font-mono mt-0.5'>{puc.vehicleNumber}</p>
              </div>
            </div>
            <button
              onClick={onClose}
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      ...
                      <p className='mt-1 text-sm font-bold text-sky-900'>
                        {puc.pucDocumentName || (isPdfDocument ? 'PUC Document PDF' : 'PUC Document Image')}
                      </p>
            ...
            <div className='bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 border-l-4 border-green-500'>
              ...
              <div className='grid grid-cols-2 md:grid-cols-4 gap-3'>
                <div className='bg-white/70 p-3 rounded-lg'>
                  ...
                  <p className='text-sm md:text-base font-bold text-gray-900 font-mono'>{puc.vehicleNumber}</p>
                </div>
                {puc.ownerName && (
                  <div className='bg-white/70 p-3 rounded-lg'>
                    <label className='text-xs font-semibold text-gray-600 block mb-1'>Owner Name</label>
                    <p className='text-sm md:text-base font-bold text-gray-900'>{puc.ownerName}</p>
                  </div>
                )}
                {puc.mobileNumber && (
                  <div className='bg-white/70 p-3 rounded-lg'>
                    <label className='text-xs font-semibold text-gray-600 block mb-1'>Mobile Number</label>
                    <p className='text-sm md:text-base font-bold text-gray-900'>{puc.mobileNumber}</p>
                  </div>
                )}
                {puc.vehicleModel && (
                  <div className='bg-white/70 p-3 rounded-lg'>
                    <label className='text-xs font-semibold text-gray-600 block mb-1'>Vehicle Model</label>
                    <p className='text-sm md:text-base font-bold text-gray-900'>{puc.vehicleModel}</p>
                  </div>
                )}
                {puc.date && (
                  <div className='bg-white/70 p-3 rounded-lg'>
                    <label className='text-xs font-semibold text-gray-600 block mb-1'>Date of Work</label>
                    <p className='text-sm md:text-base font-bold text-gray-900'>{puc.date}</p>
                  </div>
                )}
                <div className='bg-white/70 p-3 rounded-lg'>
                  <label className='text-xs font-semibold text-gray-600 block mb-1'>Valid From</label>
                  <p className='text-sm md:text-base font-bold text-gray-900'>{puc.validFrom}</p>
                </div>
                <div className='bg-white/70 p-3 rounded-lg'>
                  <label className='text-xs font-semibold text-gray-600 block mb-1'>Valid To</label>
                  <p className='text-sm md:text-base font-bold text-gray-900'>{puc.validTo}</p>
                </div>
                <div className='bg-white/70 p-3 rounded-lg'>
                  ...
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${
                    ...
                    {puc.status === 'active' ? 'Active' : puc.status === 'expiring_soon' ? 'Expiring Soon' : 'Expired'}
                  </span>
                </div>
                <div className='bg-white/70 p-3 rounded-lg'>
                  ...
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${
                    ...
                    {puc.balance > 0 ? 'Pending Payment' : 'Fully Paid'}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className='grid grid-cols-3 gap-3'>
                <div className='bg-gradient-to-br from-blue-100 to-indigo-100 rounded-lg p-3 border border-blue-300'>
                  ...
                  <p className='text-lg md:text-xl font-black text-blue-900'>
                    ₹{(puc.totalFee || 0).toLocaleString('en-IN')}
                  </p>
                </div>
                <div className='bg-gradient-to-br from-green-100 to-emerald-100 rounded-lg p-3 border border-green-300'>
                  ...
                  <p className='text-lg md:text-xl font-black text-green-900'>
                    ₹{(puc.paid || 0).toLocaleString('en-IN')}
                  </p>
                </div>
                <div className={`rounded-lg p-3 border ${
                  ...
                  <label className={`text-xs font-semibold block mb-1 ${
                    puc.balance > 0 ? 'text-orange-700' : 'text-green-700'
                  }`}>Balance</label>
                  <p className={`text-lg md:text-xl font-black ${
                    puc.balance > 0 ? 'text-orange-900' : 'text-green-900'
                  }`}>
                    ₹{(puc.balance || 0).toLocaleString('en-IN')}
                  </p>
                </div>
              </div>
            </div>
