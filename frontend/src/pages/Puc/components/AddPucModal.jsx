...
const AddPucModal = ({ isOpen, onClose, onSubmit, prefilledVehicleNumber = '', prefilledOwnerName = '', prefilledMobileNumber = '' }) => {
  ...
  const pucManualInputRef = useRef(null)
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (validPayments.length > 0 && recordId) {
          try {
            await replacePaymentsForWork('PUC', recordId, validPayments)
          } catch (paymentErr) {
            console.error('Failed to save payment received entries:', paymentErr)
            toast.warn('PUC record saved, but payment breakdown could not be saved.')
          }
        }
        ...
        if (recordId) {
          try {
            await replaceExpensesForWork('PUC', recordId, validExpenses)
          } catch (expErr) {
            console.error('Failed to save expense entries:', expErr)
            toast.warn('PUC record saved, but expense breakdown could not be saved.')
          }
        }
