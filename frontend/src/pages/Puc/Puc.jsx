...
const Puc = () => {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/puc/export`, {
        params,
        withCredentials: true,
      });

      if (!response.data.success) {
        toast.error("Failed to export PUC data", { position: "top-right", autoClose: 3000 });
        return;
      }
      ...
      if (!data || data.length === 0) {
        toast.info("No PUC records to export", { position: "top-right", autoClose: 3000 });
        return;
      }
      ...
      XLSX.writeFile(workbook, `PUC_${new Date().toISOString().split("T")[0]}.xlsx`);

      toast.success(`Exported ${data.length} PUC records`, { position: "top-right", autoClose: 3000 });
    } catch (error) {
      console.error("Error exporting PUC:", error);
      toast.error("Failed to export PUC data", { position: "top-right", autoClose: 3000 });
    }
  };
                ...
                header: {
                  ...
                  subtitle: (record) => (
                    <div className="flex flex-col">
                      {record.vehicleModel && (
                        <span className="text-gray-500 font-medium text-[11px] mt-0.5">{record.vehicleModel}</span>
                      )}
                      {record.mobileNumber && (
                        <a
                      ...
                      >
                        ...
                        <td className="px-4 2xl:px-6 py-3 2xl:py-4">
                          <div>
                            ...
                            {record.vehicleModel && (
                              <div className="text-[10px] 2xl:text-xs text-gray-500 mt-1 font-semibold">
                                {record.vehicleModel}
                              </div>
                            )}
                          </div>
                        ...
                        {/* Valid From */}
                        <td className="px-0.5 2xl:px-1 py-3 2xl:py-5 pl-8 2xl:pl-12 whitespace-nowrap">
