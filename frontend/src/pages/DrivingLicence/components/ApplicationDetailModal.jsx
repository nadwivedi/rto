    ...
    <div className='fixed inset-0 bg-black/70  z-[70] flex items-center justify-center p-2 md:p-4'>
      <div className='bg-white rounded-xl md:rounded-3xl shadow-2xl w-full md:w-[95%] lg:w-[90%] xl:w-[85%] max-h-[98vh] md:max-h-[95vh] overflow-hidden flex flex-col'>
        ...
        <div className='flex-1 overflow-y-auto p-3 md:p-5'>
          ...
          {(application.fullData?.documents?.learningLicense || application.fullData?.documents?.drivingLicense) && (
          ...
          <div className='space-y-3 md:space-y-4'>
            ...
            {/* Expense Breakdown */}
            {(application.fullData?.expenseBreakup?.filter(item => item.name && parseFloat(item.amount) > 0) || []).length > 0 && (
              <div className='bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg md:rounded-xl p-3 md:p-5 border-2 border-orange-200'>
                ...
                <div className='grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3'>
                  {(application.fullData?.expenseBreakup?.filter(item => item.name && parseFloat(item.amount) > 0) || []).map((item, index) => (
                    <div key={index} className='bg-white/80 p-2.5 md:p-3 rounded-lg border border-orange-200'>
                      <label className='text-[10px] md:text-xs font-semibold text-orange-700 uppercase'>{item.name}</label>
