...
const ApplicationDetailModal = ({ isOpen, onClose, application }) => {
  ...
  useEffect(() => {
    const recordId = application?.fullData?._id || application?._id || application?.id
    if (recordId) {
      getPaymentsByWork('DL', recordId).then(res => {
        setPaymentReceived(res.data)
      }).catch(() => setPaymentReceived([]))
      
      getExpensesByWork('DL', recordId).then(res => {
  ...
  }, [application?.fullData?._id, application?._id, application?.id])
    ...
    <div className='fixed inset-0 bg-black/70  z-[70] flex items-center justify-center p-2 md:p-4'>
      <div className='bg-white rounded-xl md:rounded-3xl shadow-2xl w-full md:w-[95%] lg:w-[90%] xl:w-[85%] max-h-[98vh] md:max-h-[95vh] overflow-hidden flex flex-col'>
        ...
        <div className='flex-1 overflow-y-auto p-3 md:p-5'>
          {/* Document Previews - LL and DL side by side */}
          {(application.fullData?.documents?.learningLicense || application.fullData?.documents?.drivingLicense || application.fullData?.documents?.otherDocument) && (
            <div className='mb-4 bg-gray-50 rounded-lg md:rounded-xl p-3 md:p-4 border-2 border-gray-200 shadow-sm'>
              ...
              <div className='flex justify-center'>
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 w-full'>
                  ...
                  {/* Other Document */}
                  {application.fullData?.documents?.otherDocument && (
                    <div className='bg-white rounded-lg border border-gray-200 p-3 shadow-sm'>
                      <p className='text-xs font-bold text-teal-700 mb-2 flex items-center gap-1 justify-center'>
                        <svg className='w-3.5 h-3.5' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                          <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13' />
                        </svg>
                        Other Document
                      ...
                      <div className='flex justify-center'>
                        {application.fullData.documents.otherDocumentType === 'application/pdf' ? (
                          <button
                            onClick={() => handleViewPdf(application.fullData.documents.otherDocument)}
                            className='flex items-center gap-2 px-3 py-1.5 bg-teal-50 text-teal-700 hover:bg-teal-100 border border-teal-200 rounded-md text-xs font-semibold transition-colors shadow-sm'
                          >
                            <svg className='w-4 h-4' fill='currentColor' viewBox='0 0 20 20'>
                              <path fillRule='evenodd' d='M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z' clipRule='evenodd' />
                            </svg>
                          ...
                          <img
                            src={application.fullData.documents.otherDocument}
                            alt='Other Document'
                            onClick={() => setIsImageModalOpen('other')}
                            className='max-w-full h-32 md:h-40 object-contain rounded cursor-pointer border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 hover:scale-105'
                            title='Click to view full size'
                          />
                        )}
          ...
          <div className='space-y-3 md:space-y-4'>
            ...
            {/* Row 1: License Class + By/Referral */}
            <div className='grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4'>
              ...
              {/* By / Referral */}
              <div className='bg-gradient-to-br from-cyan-50 to-blue-50 rounded-lg md:rounded-xl p-3 md:p-4 border-2 border-cyan-200'>
                ...
                {(application.fullData?.byName || application.fullData?.byMobile) ? (
                  <div className='grid grid-cols-2 gap-2'>
                    {application.fullData?.byName && (
                      <div className='bg-white/80 p-2 rounded-lg'>
                        <label className='text-[10px] md:text-xs font-semibold text-gray-600'>Agent / Referral Name</label>
                        <p className='text-xs md:text-sm font-bold text-cyan-900 mt-0.5'>{application.fullData.byName}</p>
                      </div>
                    )}
                    {application.fullData?.byMobile && (
                      <div className='bg-white/80 p-2 rounded-lg'>
                        <label className='text-[10px] md:text-xs font-semibold text-gray-600'>Mobile Number</label>
                        <p className='text-xs md:text-sm font-bold text-cyan-900 mt-0.5'>{application.fullData.byMobile}</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className='bg-white/60 p-2 rounded-lg'>
                    <p className='text-xs text-gray-400 font-medium italic'>No referral details provided</p>
                  </div>
                )}
              </div>
            </div>

            {/* Row 2: LL Details + DL Details */}
            <div className='grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4'>
              {/* Learning License Information */}
              <div className='bg-gradient-to-br from-yellow-50 to-orange-50 rounded-lg md:rounded-xl p-3 md:p-4 border-2 border-yellow-200'>
            ...
            {(expenseItems?.filter(item => item.name && parseFloat(item.amount) > 0) || []).length > 0 && (
              <div className='bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg md:rounded-xl p-3 md:p-5 border-2 border-orange-200'>
                <h3 className='text-sm md:text-base font-bold text-orange-900 mb-2 md:mb-3 flex items-center gap-1.5 md:gap-2'>
                  <svg className='w-3.5 h-3.5 md:w-4 md:h-4 text-orange-600' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                    <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z' />
                  </svg>
      ...
      {isImageModalOpen && (
        ...
        >
          ...
          {(() => {
            const docMap = {
              ll: { src: application.fullData?.documents?.learningLicense, label: 'Learning License' },
              dl: { src: application.fullData?.documents?.drivingLicense, label: 'Driving License' },
              other: { src: application.fullData?.documents?.otherDocument, label: 'Other Document' }
            }
            const currentDoc = docMap[isImageModalOpen] || docMap.ll
            return (
              <img 
                src={currentDoc.src}
                className='max-w-full max-h-full object-contain rounded-lg shadow-2xl select-none'
                onClick={(e) => e.stopPropagation()} 
                alt={`Full Size ${currentDoc.label}`}
              />
            )
          })()}
        </div>
