...
const Insurance = () => {
  ...
  const handleSmsClick = (insurance) => {
    if (!insurance.mobileNumber || insurance.mobileNumber === 'N/A') {
      toast.error('Mobile number not available for this record', { position: 'top-right', autoClose: 3000 });
      return;
    }

    const phoneNumber = insurance.mobileNumber.replace(/\D/g, '').slice(-10);
    const effectiveStatus = getEffectiveStatus(insurance);
    const isThirdParty = effectiveStatus !== insurance.status;
    const expiryField = isThirdParty ? 'Third Party Insurance' : 'Insurance';
    const expiryDate = isThirdParty ? insurance.thirdPartyValidTo : insurance.validTo;
  ...
  // SMS icon only for expiring soon / expired insurance
  const shouldShowSmsButton = (insurance) => {
    const effStatus = getEffectiveStatus(insurance);
    return effStatus === 'expiring_soon' || effStatus === 'expired';
  };
