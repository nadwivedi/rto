...
const AddTaxModal = ({ isOpen, onClose, onSubmit, prefilledVehicleNumber = '', prefilledOwnerName = '', prefilledMobileNumber = '' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate payment breakdown doesn't exceed total amount
    const totalReceived = paymentReceived.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0)
    if (totalReceived > parseFloat(formData.totalAmount || 0)) {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (validPayments.length > 0 && recordId) {
          try {
            await replacePaymentsForWork('TAX', recordId, validPayments)
          } catch (paymentErr) {
            console.error('Failed to save payment received entries:', paymentErr)
            toast.warn('Tax record saved, but payment breakdown could not be saved.')
          }
        }
        ...
        if (recordId) {
          try {
            await replaceExpensesForWork('TAX', recordId, validExpenses)
          } catch (expErr) {
            console.error('Failed to save expense entries:', expErr)
            toast.warn('Tax record saved, but expense breakdown could not be saved.')
          }
        }
