...
const AddSpeedGovernorModal = ({ isOpen, onClose, onSubmit, prefilledVehicleNumber = '', prefilledOwnerName = '', prefilledMobileNumber = '' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/api/speed-governor`, dataToSubmit, {
      ...
      if (response.data.success) {
        ...
        if (validPayments.length > 0 && recordId) {
          try {
            await replacePaymentsForWork('SPEED_GOVERNOR', recordId, validPayments)
          } catch (paymentErr) {
            console.error('Failed to save payment received entries:', paymentErr)
            toast.warn('Speed Governor saved, but payment breakdown could not be saved.')
          }
        }
        ...
        if (recordId) {
          try {
            await replaceExpensesForWork('SPEED_GOVERNOR', recordId, validExpenses)
          } catch (expErr) {
            console.error('Failed to save expense entries:', expErr)
            toast.warn('Speed Governor saved, but expense breakdown could not be saved.')
          }
        }

        toast.success('Speed Governor record added successfully!')
      ...
      <div className='bg-white rounded-xl md:rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] overflow-hidden flex flex-col'>
        <div className='bg-gradient-to-r from-orange-600 to-red-600 p-2 md:p-3 text-white flex-shrink-0'>
          <div className='flex items-center justify-between gap-3'>
            <div>
              <h2 className='text-lg md:text-2xl font-bold'>Add Speed Governor</h2>
              <p className='text-orange-100 text-xs md:text-sm mt-1'>Add speed governor work record</p>
            </div>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col flex-1 overflow-hidden'>
          <div className='flex-1 overflow-y-auto p-3 md:p-6'>
            ...
            <div className='bg-gradient-to-r from-emerald-50 to-teal-50 border-2 border-emerald-200 rounded-xl p-3 md:p-6 mb-4 md:mb-6'>
              ...
              <div className='grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4'>
                ...
                <div>
                  ...
                  />
                  {paidExceedsTotal && (
                    <p className='text-xs mt-1 text-red-600 font-semibold'>Paid amount cannot exceed total fee!</p>
                  )}
                </div>
            ...
            <div className='bg-gradient-to-r from-slate-50 to-gray-50 border-2 border-slate-200 rounded-xl p-3 md:p-6'>
              ...
              <textarea
                ...
                rows={3}
                className='w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-transparent bg-white resize-none'
              />
            </div>
          ...
          <div className='border-t border-gray-200 p-3 md:p-4 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-3 flex-shrink-0'>
            ...
            <div className='flex gap-2 md:gap-3 w-full md:w-auto'>
              ...
              <button type='submit' disabled={isSubmitting} className='flex-1 md:flex-none px-6 md:px-8 py-2 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-lg hover:shadow-lg font-semibold transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed'>
                {isSubmitting ? (
                  <>
...
export default AddSpeedGovernorModal
